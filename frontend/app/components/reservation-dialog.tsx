...
// Simple UI components to replace complex Radix ones for MVP stability
...
export function ReservationDialog({ open, onOpenChange, parking, onConfirm }: ReservationDialogProps) {
    ...
    const handleConfirm = () => {
        ...
        // Validation for 0.5 increments
        if (!isDayPass) {
            const d = parseFloat(duration);
            if (d <= 0 || (d * 10) % 5 !== 0) { // Check if divisible by 0.5
                // Validation logic (optional alert)
            }
        }

        const startDateTime = new Date(`${date}T${startTime}`);
        // For Day Pass, duration is effectively 24 hours from start or until end of day
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
            <div className="bg-white rounded-lg shadow-lg w-full max-w-md p-6 m-4 animate-in fade-in zoom-in-95 duration-200">
                        ...
                        {vehicles.length > 0 ? (
                            ...
                            <div className="bg-yellow-50 p-4 rounded-lg border border-yellow-200 text-center">
                                ...
                                <Button
                                    ...
                                    onClick={() => {
                                        onOpenChange(false);
                                        // Use window.location as fallback or pass router prop if useRouter fails in modal context,
                                        // but useRouter should work.
                                        // We need to import useRouter first.
                                        window.location.href = "/?mode=driver&tab=vehicles";
                                    }}
                                >
                                    차량 등록하러 가기
                                </Button>
                ...
                <div className="mt-6 flex justify-end gap-2">
                    ...
                    <Button
                        ...
                        disabled={!date || !startTime || (!isDayPass && !duration) || !carNumber}
                    >
                        예약 확인
                    </Button>
                </div>
